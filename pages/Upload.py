...
if dataPredict is not None:
    ...
    if button:
        ...
        url = "http://1203f1e8-b7e1-4b6d-906c-c676a75aea1b.southeastasia.azurecontainer.io/score"
        payload = json.dumps(kirim)
        headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer A8rD4mryl3y1KJ0ioFAgNvDZCmELlOhB'
        }

        response = requests.request("POST", url, headers=headers, data=payload)
        hasil = response.json()
        dfhasil = pd.DataFrame(hasil["Results"]["WebServiceOutput0"])
        dfhasil = dfhasil.drop(columns=["Churn"])
        st.write(dfhasil)
        st.write(dfhasil["Scored Probabilities"].describe())
